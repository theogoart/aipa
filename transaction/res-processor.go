...
import (
	...
	"errors"
	"fmt"
	"github.com/aipadad/aipa/common/safemath"
	"strings"
)
...
func ProcessSpaceResource(roleIntf role.RoleInterface, trx *types.Transaction, dbUseSize uint64, checkMinBala bool) (uint64, role.ResourceUsage, error, aipaErr.ErrCode) {
	...
	if bErr != aipaErr.ErrNoError {
		log.Errorf("RESOURCE:GetTxSize error:%v", aipaErr.GetCodeString(bErr))
	...
	if err != nil {
		...
		return 0, usage, err, aipaErr.ErrNoError
	}
	...
	if ufsl.Available+usl.Available < resService.resConfig.MinAccountSpaceBalance {
		...
		return 0, usage, nil, aipaErr.ErrTrxCheckMinSpaceError
	}

	if ufsl.Available+usl.Available < txSize {
		log.Errorf("RESOURCE:CheckSpace failed,Account:%v, space size:%v, userSpaceAllAvailable:%v, error:%v", acc, txSize, ufsl.Available+usl.Available, aipaErr.GetCodeString(aipaErr.ErrTrxCheckSpaceError))
		return 0, usage, nil, aipaErr.ErrTrxCheckSpaceError
	}
	...
	if txSize <= usl.Available {
		...
		return txSize, usage, err, aipaErr.ErrNoError
	} else {
		...
		return txSize, usage, nil, aipaErr.ErrNoError
	}
}
...
func GetTxSize(resConfig role.ResourceConfig, trx *types.Transaction, dbUseSize uint64) (uint64, aipaErr.ErrCode) {
	tb, err := bpl.Marshal(trx)
	if err != nil {
		log.Errorf("RESOURCE: marshal failed:%v", err)
		return 0, aipaErr.RestErrBplMarshal
	}
	...
	if strings.Contains(trx.Method, "deploy") {
		if l > resConfig.MaxSpacePerDeployTrx {
			...
			return l, aipaErr.ErrTrxResourceExceedMaxSpacePerTrx
		}
		return l, aipaErr.ErrNoError
	}
...
func ProcessTimeResource(roleIntf role.RoleInterface, trx *types.Transaction, txTime uint64, checkMinBala bool) (uint64, role.ResourceUsage, error, aipaErr.ErrCode) {
	...
	if ufsl.Available+usl.Available <= resService.resConfig.MinAccountTimeBalance {
		...
		return 0, usage, nil, aipaErr.ErrTrxCheckTimeError
	}

	if ufsl.Available+usl.Available < txTime {
		log.Errorf("RESOURCE:CheckTime failed, Account:%v, time size:%v, TimeAllAvailable:%v, error:%v", acc, txTime, ufsl.Available+usl.Available, aipaErr.GetCodeString(aipaErr.ErrTrxCheckTimeError))
		return 0, usage, nil, aipaErr.ErrTrxCheckTimeError
	}
	...
	if txTime <= usl.Available {
		...
		return txTime, usage, err, aipaErr.ErrNoError
	} else {
		...
		return txTime, usage, nil, aipaErr.ErrNoError
	}
}
...
func MaxAvailableTime(resService *ResProcessorService, acc string, checkMinFlag bool) (Limit, Limit, error) {
	...
	if acc == config.aipa_CONTRACT_NAME {
...
func GetTotalBalance(role role.RoleInterface, sender string) (*big.Int, error) {
	...
	result := big.NewInt(0)
	if sender == config.aipa_CONTRACT_NAME {
