...
func NativeContractInitChain(ldb *db.DBService, roleIntf role.RoleInterface, ncIntf NativeContractInterface) ([]*types.Transaction, error) {
	...
	for i = 0; i < len(config.Genesis.InitDelegates); i++ {
		...
		nparam, err2 := abi.MarshalAbiEx(mapstruct, Abi, config.aipa_CONTRACT_NAME, "newaccount")
		if err2 != nil {
			log.Error("abi.MarshalAbiEx failed for new account:", name)
			continue
		}

		trx := newTransaction(config.aipa_CONTRACT_NAME, "newaccount", nparam)
		trxs = append(trxs, trx)

		// 2, transfer trx

		mapstruct2 := make(map[string]interface{})
		abi.Setmapval(mapstruct2, "from", config.aipa_CONTRACT_NAME)
		...
		tparam, err3 := abi.MarshalAbiEx(mapstruct2, Abi, config.aipa_CONTRACT_NAME, "transfer")
		if err3 != nil {
		...
		trx = newTransaction(config.aipa_CONTRACT_NAME, "transfer", tparam)
		trxs = append(trxs, trx)
		...
		sparam, err4 := abi.MarshalAbiEx(mapstruct3, Abi, config.aipa_CONTRACT_NAME, "setdelegate")
		if err4 != nil {
			log.Info("abi.MarshalAbiEx failed for setdegelage with account:", name)
			continue
		}
		trx = newTransaction(config.aipa_CONTRACT_NAME, "setdelegate", sparam)
		trxs = append(trxs, trx)
	}
...
func CreateNativeContractAccount(roleIntf role.RoleInterface) error {
	// account
	_, err := roleIntf.GetAccount(config.aipa_CONTRACT_NAME)
	if err == nil {
		return nil
	}
	...
	bto := &role.Account{
		AccountName: config.aipa_CONTRACT_NAME,
	...
	initSupply, err = safemath.U256Mul(initSupply, new(big.Int).SetUint64(config.aipa_INIT_SUPPLY), new(big.Int).SetUint64(config.aipa_SUPPLY_MUL))
	if err != nil {
		return err
	}
